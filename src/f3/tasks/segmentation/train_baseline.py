...
from f3.tasks.segmentation.utils import (EventSegformer,
...
def main():
    ...
    if args.name is None:
        args.name = f"f3segbl_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    if resume:
        last_dict = torch.load(f"{base_path}/models/last.pth")
        ...
        try:
            best_dict = torch.load(f"{base_path}/models/best.pth")
