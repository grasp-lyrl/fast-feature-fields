...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str,
                        default="src/f3/tasks/depth/checkpoints/depth_anything_v2_vitl.pth",
    ...
    ovc_key = '/ovc/rgb'
    if ovc_key not in h5f:
        ovc_key = f'/ovc/{args.side}'
    ...
    if args.out_h5fn is None:
        args.out_h5fn = args.h5fn.replace("data.h5", f"dav2_disp_depth_{args.side}.h5")
    ...
    for idx in tqdm(range(total_images), total=total_images):
        ...
        model_depth_im = model.infer_image(ovc_image)
        model_depth_im_remapped = remap_and_mask(source_to_target_map, source_to_target_mask, model_depth_im)
        ...
        masked_result = model_depth_im_remapped.round().astype(np.uint16)
        masked_result[mask] = 2 ** 16 - 1

        # max_depth = np.max(model_depth_im)
        # depth_im_colored = cmap(model_depth_im.squeeze() / max_depth) * 255
        # depth_im_colored = depth_im_colored.astype(np.uint8)
        # cv2.imwrite("depth_im_colored.png", depth_im_colored)
        # # cv2.imwrite("model_depth_im.png", model_depth_im.round().astype(np.uint8))
        # # cv2.imshow("masked_result", masked_result.round().astype(np.uint8))
        # cv2.imwrite("ovc_image.png", ovc_image)
        # cv2.waitKey(1)
