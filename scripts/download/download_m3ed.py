...
if __name__ == "__main__":
    ...
    parser.add_argument("--sequence", required=False,
                        help="Name of specific sequence to download. " +
                             "If provided, only this sequence will be downloaded " +
                             "(overrides vehicle and environment filters)")
    ...
    if args.sequence:
        print(f"Specific sequence: {args.sequence}")
    else:
    ...
    for file in dataset_list:
        ...
        # If specific sequence is requested, only match that sequence
        if args.sequence:
            if filename != args.sequence:
                continue
        else:
